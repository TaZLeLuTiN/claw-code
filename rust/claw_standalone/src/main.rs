    ...
    match cli.command {
        ...
        Commands::Chat { message } => {
            chat_with_context(
                cli.project.as_ref(),
                &message,
                cli.model.as_ref().map(|x| x.as_str()),
            )
            .await?;
        }
...
fn create_bmad_files(project_path: &PathBuf, language: &str) -> Result<()> {
    ...
    let instructions = format!(
        r#"# Instructions fondamentales du projet {}
...
"#,
    ...
    fs::write(
        project_path.join(".claw-framework/INSTRUCTIONS.md"),
        instructions,
    )?;
...
"#;
    ...
    // MANIFEST.md
    let manifest = format!(
        r#"# Manifest du projet - Historique immuable
...
fn load_project_context(project_path: &PathBuf) -> Result<String> {
    ...
    let instructions = fs::read_to_string(framework_dir.join("INSTRUCTIONS.md"))
        .unwrap_or_else(|_| "# No instructions found".to_string());
    let manifest = fs::read_to_string(framework_dir.join("MANIFEST.md"))
