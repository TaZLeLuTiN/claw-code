...
use super::{AIProvider, ChatRequest, ChatResponse, ProviderError, Usage};
        ...
        if response.status().is_success() {
            ...
            let model_exists = models["models"]
                ...
                .map(|models| {
                    models
                        .iter()
                        .any(|model| model["name"].as_str().unwrap_or("").contains(&self.model))
                })
                .unwrap_or(false);
...
impl AIProvider for OllamaProvider {
    ...
    async fn health_check(&self) -> Result<(), ProviderError> {
        ...
        match response {
            ...
            Ok(_) => Err(ProviderError::Network(
                "Ollama service not responding correctly".to_string(),
            )),
